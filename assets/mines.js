...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-e04ef48b";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-e04ef48b">'+'<span class="tile-face-e04ef48b tile-front-e04ef48b"></span>'+'<span class="tile-face-e04ef48b tile-back-e04ef48b"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-e04ef48b")){return;}
var back=tile.querySelector(".tile-back-e04ef48b");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-e04ef48b");if(isMine){tile.classList.add("boom-e04ef48b");}
if(dim){tile.classList.add("dimmed-e04ef48b");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-e04ef48b")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-e04ef48b");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-e04ef48b")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-e04ef48b");void panelEl.offsetWidth;panelEl.classList.add("jolt-e04ef48b");}
endRound();}
